...
echo "test 4"
echo "Use: ./<program> <FileName> s1 s2" > $EXPECTED
./$BIN 2> $OUTFILE
diff $EXPECTED $OUTFILE
...
echo "test 13"
echo "Error opening file" > $EXPECTED
./$BIN err$INFILE "1" "2" 2> $OUTFILE
diff $EXPECTED $OUTFILE

echo "test 14"
echo "Error opening file" > $EXPECTED
chmod 000 $INFILE
./$BIN $INFILE "1" "2" 2>$OUTFILE
chmod 644 $INFILE
diff $EXPECTED $OUTFILE
...
echo "test 20"
echo "Use: s1 cannot be empty" > $EXPECTED
./$BIN $INFILE "" "c" 2> $OUTFILE
diff $EXPECTED $OUTFILE
